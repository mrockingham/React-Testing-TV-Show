...
import App from './App'
import { render, fireEvent, waitFor, act, wait } from  '@testing-library/react'
...
test.todo('dropdown menu'), async () => {

    
    
    fetchShow.mockResolvedValueOnce(episodes)
    const {getByText, getAllBuText} = render(<App />)

    await wait(() =>{
        getByText(/choose season/i)

    })
    userEvent.click(getByText(/choose season/i))
